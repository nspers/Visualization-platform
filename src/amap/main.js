...
export class WorkCover {
  ...
  addBillboard(cor, opt, obj) {
    ...
    marker.on("click", function(e) {
      ...
      // 防止重复点击
      if (
        self.lastClickedMarkers.length > 0 &&
        data.id === self.lastClickedMarkers[0].getExtData().id
      ) {
        return;
      }
  ...
  getBillboardById(id) {
    ...
    var marker = window.amap.markers.find(
  ...
  changeBillBoardstatusAttr(id, status, selected, textClass) {
    ...
    if (marker) {
      if (status) {
        ...
        status = marker.getExtData().opt.status;
      }
  ...
  updateSelById(id) {
    this.changeBillBoardstatusAttr(id, null, "isSelect");
  }
  destroy() {
    if (this.cluster) {
