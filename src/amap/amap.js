export default class AMapTool {
  ...
  static async loadAndInit(param) {
    await new Promise((resolve, reject) => {
      if (window.AMap) {
        ...
        script.src =
          "//webapi.amap.com/maps?v=1.4.15&key=a3a9810bca5fbdb3fee6cd7c77fcd927&callback=AMapCallBack&plugin=AMap.MarkerClusterer";
  ...
  constructor({
      ...
      getTileUrl: function(a, r, c) {
        ...
        1 === l
          ...
          ? ((d = a - t * s - o * (2 * (l - 1))),
            (u = r - n * s - i * (2 * (l - 1))))
          : 4 === l &&
            ((d = a - t * s - o * (2 * l)), (u = r - n * s - i * (2 * l)));
        var m =
          "http://localhost:9999/yangyanlu/map" +
          l +
          "/" +
          d +
          "," +
    ...
    this.satellite = new AMap.TileLayer({
      getTileUrl:
        "https://mt{1,2,3,0}.google.cn/vt/lyrs=s&hl=zh-CN&gl=cn&x=[x]&y=[y]&z=[z]&s=Galile"
    });
    ...
    limit &&
      this.setLimitBounds([119.734476, 34.140279], [119.833653, 34.193719]);
    ...
    this.cluster = new AMap.MarkerClusterer(this.amap, this.markers, {
      ...
      renderClusterMarker: function(context) {
        ...
        context.marker.setContent(
          `<div class="amap-marker-cluster">${context.count}</div>`
        );
      }
    });
    return this.cluster;
...
function createMarkerHTML({
  ...
  if (text) {
    html.push(
      `<div class="amap-marker-text ${
        textClass ? textClass : ""
      }">${text}</div>`
    );
  }
  html.push(
    `<img class="amap-marker-image" width="${imageWidth ||
      "auto"}" height="${imageHeight || "auto"}" src="${imageUrl}" /></div>`
  );
